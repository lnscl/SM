...
valBarley <- 191 #euro/1000kg - with support.
valMaize <- 181 #euro/1000kg - with support.
cosBarley <- 400 #euro/ha.
cosMaize <- 1438 #euro/ha.
...
bbr <- lapply(1:length(crops), function(i){
  ...
  # who plants LCM
  whoLCM <- data.frame(
  R1 = ifelse(ci$LCM > (T1 * (R2 + R3) + 0.001), T1 * R1, 0),
  R2 = ifelse(ci$LCM > (T1 * R3), T1 * R2, 0),
  R3 = ifelse(ci$LCM > 0, T1 * R3, 0))
...
whoSCB <- data.frame(
  R1 = ifelse(ci$SCB > (T1 * R2) + 0.001, T1 * R1, 0),
  R2 = ifelse(ci$SCB > 0, T1 * R2, 0))
...
bSCB1 <- yi$SCB * whoSCB$R1/whoSCB$total * valBarley - cosBarley * whoSCB$R1 * total_area_third
bLCM1 <- yi$LCM * whoLCM$R1/whoLCM$total * valMaize - cosMaize * whoLCM$R1 * total_area_third
bR1 <- bLCB1 + ifelse(is.na(bSCB1), 0, bSCB1) + ifelse(is.na(bLCM1), 0, bLCM1) 
## R2:
bSCB2 <- yi$SCB * whoSCB$R2/whoSCB$total * valBarley - cosBarley * whoSCB$R2 * total_area_third
bLCM2 <- yi$LCM * whoLCM$R2/whoLCM$total * valMaize - cosMaize * whoLCM$R2 * total_area_third
bR2 <- ifelse(is.na(bSCB2), 0, bSCB2) + ifelse(is.na(bLCM2), 0, bLCM2)
## R3:
bLCM3 <- yi$LCM * whoLCM$R3/whoLCM$total * valMaize - cosMaize * whoLCM$R3 * total_area_third
bR3 <- ifelse(is.na(bLCM3), 0, bLCM3)
## T2
...
vtable <- data.frame(bLCB1, bSCB1, bLCM1, bSCB2, bLCM2, bLCM3, bR1, bR2, bR3, bT2, totalT1 = bR1 + bR2 + bR3)/1000000
vtable$total <- vtable$totalT1 + bT2/1000000

# table to return
rtable <- data.frame(bR1, bR2, bR3, bT2)/1000000
...
# Use R1 P and Fp for R2 and R3 too as all would follow the R1 options with perfect information
TBu <- TB
TBu$R2[TBu$information == "P"] <- TB$R1[TB$information == "P"]
TBu$R3[TBu$information == "P"] <- TB$R1[TB$information == "P"]
TBu$R2[TBu$information == "Fp"] <- TB$R1[TB$information == "Fp"]
TBu$R3[TBu$information == "Fp"] <- TB$R1[TB$information == "Fp"]

TB2 <- melt(TBu)
# add column with the value per hectare
TB2$value_ha <- rep(0, dim(TB2)[1])
TB2$value_ha[TB2$variable == "R1"] <- TB2$value[TB2$variable == "R1"]/(total_area_third * T1 * R1)*1000000
TB2$value_ha[TB2$variable == "R2"] <- TB2$value[TB2$variable == "R2"]/(total_area_third * T1 * R2)*1000000
TB2$value_ha[TB2$variable == "R3"] <- TB2$value[TB2$variable == "R3"]/(total_area_third * T1 * R3)*1000000
...
ptrv2 <- ggplot(rvs, x=value, y=factor(valRun, levels = c("bsl", "M2", "B2"), ordered =T), aes(x = information, y = value, group = variable, fill = information)) +
  geom_col_pattern(position = "dodge_2", aes(pattern = valRun), colour = "black", pattern_density = 0.1, pattern_spacing = 0.02)+
